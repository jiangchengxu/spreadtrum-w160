// DlgNetProfile.cpp : implementation file
//
...
CDlgNetProfile::CDlgNetProfile(CWnd* pParent /*=NULL*/)
    : CBaseDialog(CDlgNetProfile::IDD, pParent)
{
    //	m_nBkTag = 1;
...
void CDlgNetProfile::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDlgNetProfile, CBaseDialog)
...
/////////////////////////////////////////////////////////////////////////////
// CDlgNetProfile message handlers

BOOL CDlgNetProfile::OnInitDialog()
{
...
BOOL CDlgNetProfile::OnPaint()
{
    CBaseDialog::OnPaint();
}

void CDlgNetProfile::OnSelchangeComboConnectName()
{
...
void CDlgNetProfile::OnButtonSaveconn()
{
    // TODO: Add your control notification handler code here
...
void CDlgNetProfile::OnButtonNewconn()
{
    // TODO: Add your control notification handler code here
    UpdateData(TRUE);
    ...
    if (0 == m_connect_type) {
        CDlgInternetNewConn dlg(NULL, 0);
        if(dlg.DoModal() == IDOK) {
            StConnProfile profile;
            memset(&profile, 0x00, sizeof(StConnProfile));
            ...
            wcscpy(profile.szAPN, dlg.m_strAPN);
        ...
        CDlgInternetNewConn dlg(NULL, 1);
        if(dlg.DoModal() == IDOK) {
            StConnProfile profile;
            memset(&profile, 0x00, sizeof(StConnProfile));
            profile.bDefault = dlg.m_bSetDefault;
...
void CDlgNetProfile::OnButtonDeleteconn()
{
    UpdateData(TRUE);
    int nIndex = m_cmbConnectionName.GetCurSel();
...
BOOL CDlgNetProfile::SndAtCGDCONT(char* strConnPoint)
{
...
void CDlgNetProfile::RspAtCGDCONT(LPVOID pWnd, BYTE (*strArr)[DSAT_STRING_COL], WORD wStrNum)
{
    CDlgNetProfile* pDlg = (CDlgNetProfile*)pWnd;
...
LRESULT CDlgNetProfile::OnUpdateInternetDlg(WPARAM wParam, LPARAM lParam)
{
    int nIndex = m_cmbConnectionName.GetCurSel();

    switch(wParam) {
    case TRUE:
        //GetDlgItem(IDC_COMBO_CONNECT_NAME)->EnableWindow(0 != nIndex);
        GetDlgItem(IDC_EDIT_PHONE_NUMBER)->EnableWindow(0 != nIndex);
...
void CDlgNetProfile::UpdateButton()
{
    if(m_pConnProf->IsEmpty()) {
        ...
        GetDlgItem(IDC_EDIT_APN)->EnableWindow(TRUE);
...
void CDlgNetProfile::SetAPN(CString strPLMN)
{
    if(BApnSet) { //已经设置过
        return;
    }
...
void CDlgNetProfile::OnButtonDetails()
{
    // TODO: Add your control notification handler code here
...
/* ndis support*/
#ifdef FEATURE_NDIS_SUPPORT
void CDlgNetProfile::OnRadioNdis()
{
    UpdateData(TRUE);
    ...
    if (nConnNum == 0) {
        ...
        strTempDefault .Format(_T("%s (Default)"), g_SetData.Internet_szDefaultConnName);
...
void CDlgNetProfile::OnRadioRas()
{
    UpdateData(TRUE);
